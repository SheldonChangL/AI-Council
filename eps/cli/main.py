"""eps Typer CLI 入口（Story 6.1 / FR-12 / 藍圖 T4.3）。

提供兩個子命令，皆經既有 REST API 操作，CLI 本身不含業務規則：

- ``run``：以 ``POST /sessions`` 建立研討會話並印出 ``sessionId``（AC-2）。
- ``status``：以 ``GET /sessions/{id}`` 查詢並印出目前狀態（AC-3）。

執行：``python -m eps.cli.main --help`` 列出子命令（AC-1）。
